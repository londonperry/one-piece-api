...
const getCharacterInfo = async (characterName: string): Promise<Prisma.op_characterCreateInput> => {
...
const loadCharacters = async () => {
    try {
        ...
        const characters: Prisma.op_characterCreateInput[] = await Promise.all(characterInfoPromises);
        console.log("🚀 ~ file: seed.ts ~ line 138 ~ loadCharacters ~ characters", characters)
        // save them to the db
        console.log("Let's seed it");
        await prisma.op_character.createMany({ data: characters });
    } catch (error) {
        console.error(error)
    }
};
